...
class MessagesController extends Controller {
    ...
    public function search(Request $request)
    {
        ...
        if (Auth::user()->client_parent == null)
        {
            $records = User::where('name', 'LIKE', "%{$input}%")->where('client_parent', Auth::user()->id)->where('id','!=',Auth::user()->id);
        }
        else
        {
            $records = User::where('name', 'LIKE', "%{$input}%")->where('client_parent', Auth::user()->client_parent)->where('id','!=',Auth::user()->id)->orwhere('id', Auth::user()->client_parent);
        }
        ...
        // send the response
        return Response::json([
                    'records' => $records->count() > 0 ? $getRecords : '<p class="message-hint center-el"><span>Nothing to show.</span></p>',
                    'addData' => 'html'
                        ], 200);
    }
